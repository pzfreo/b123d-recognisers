...
- Defining-face audit:
  [`e2-angled-step-frame-losses.json`](e2-angled-step-frame-losses.json), generated by
  `tools/audit_angled_step_frame_losses.py`; artifact SHA-256
  `f1d527e4bc6b3771c6f8922e914cdd890901941499e33152038e303470173dcb`.
...
The focused audit makes that disposition reproducible rather than inferring it from counts. It
identifies raw accepted defining faces `14` in `10492` and `2`, `8`, `19` in `10649`, validates
each face across normalization by type, edge count, area, transformed centre and transformed
normal, and runs that same local face through the production discovery gates. Faces `8` and `19`
become three-axis (`compound`) slants. Faces `14` and `2` retain a single run axis, but neither has
the two principal local neighbour planes required for a supported corner bevel. Direct and
aggregate local recognition both return zero, proving recognizer rejection rather than a
correspondence or scorer loss. The artifact also pins each source STEP SHA-256 and the complete
inferred frame basis.
...
re-import retains the same record. The focused module has 25 passing tests.
