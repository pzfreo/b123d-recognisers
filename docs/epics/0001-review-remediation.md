...
- [~] Annotate the internal geometry helpers, then enable `disallow_untyped_defs`. **43 return
      ...
      has one.** Parameters: the "231" first reported included 34 `self`/`cls`, which never need
      annotating — the real figure was **197, now 89**. Done in `_geometry`, `countersinks`,
      `flats`, `polygonal_bosses`, `_recess_core`, `_hole_features` and the pattern modules,
      using the `_typing` aliases that already existed (`Vector3`, `Span2`, `Bounds`,
      `FaceLike`, `Part`) rather than inventing new ones, plus one new internal type:
      `SegmentEvidence`, a `CylinderEvidence` widened with the patch faces a segment absorbed.
      It is internal, so it lives in `_hole_features` rather than the published `_typing`.
