...
| `recognise_passages` | Writer-free compatibility projection of principal-axis, line-polygon `SectionPassage` geometry into the unchanged schema-v1 `Passage` values and order. | Attributed calls fail loudly from 0.4.0; use `recognise_section_passages(..., ledger=...)`. Oblique rich occurrences have no legacy value. | Exact legacy semantic goldens plus the 0.4 migration/error boundary tests. |
