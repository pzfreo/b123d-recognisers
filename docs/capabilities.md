...
| `recognise_section_passages` | Constant-section line-walled passages on principal or free axes, open at both ends. A complete equal-span wall cycle is preferred; an interrupted cycle may instead be proved by exactly two matching convex polygonal mouths and one same-solid bounded enclosure. Mouth-adjacent planar walls are defining and the traversed region is constituent membership. | Capped, tapered, stepped, curved-mouth, one-ended, open/invalid, cross-solid, multiply opened, or materially obstructed cavities; line/arc schema exists but arc-wall discovery is deferred. | Independent topology oracle, principal compatibility matrix, interrupted triangular/rectangular/six-sided goldens, compound and ambiguity negatives, arbitrary rigid transforms, STEP, exact Candidate/evidence tests, and MFCAD++ development evidence. |
