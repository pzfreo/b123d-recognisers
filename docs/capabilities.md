...
| `recognise_plates` | Body-local thin prismatic slabs supported by opposed planar face clusters, a roll-invariant body-oriented cross-envelope area gate, and the configured thickness gate. Equal-valued occurrences on separate valid solids retain multiplicity and independent transverse witnesses. | The single envelope plate, curved/non-prismatic shells, internally oblique normals, cross-solid face pairing, and slabs below the evidence gates. | Plate/level golden; signed-axis and in-plane-roll boundaries; single/equal/unequal/nested compound, framed rigid-motion, STEP, provenance and package tests; paired [census and MFCAD++-500 body-locality evidence](benchmarks/e2-plate-body-locality-validation.md). |
