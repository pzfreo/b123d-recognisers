...
| --- | --- | --- | --- |
| `recognise_blends` | Complete same-solid convex or concave chains of native cylindrical rolling-ball patches with one analytic radius, one proved material side, two complete support regions and complete original-face evidence. Concave projection requires supports that can form an edge: two proved parallel planes identify a circular slot end instead. One chain is one occurrence; small and non-principal chains remain visible. Concave internal rounds may coexist with the enclosing Pocket/Slot/Step's constituent evidence. Aggregate reconciliation removes a convex chain only when accepted Fillet occurrences cover all of its curved faces. | Parallel-wall circular ends; incomplete, branching, cyclic, mixed-radius/side, ambiguous-support or cross-solid components; tori, spheres, surfaces of revolution and recovered/unoriented cylinders. CircularBlindStep and annular Boss/Hole geometry do not satisfy the chain contract. | Authored convex/concave, obround negative, small-radius, oblique, scale, compound, traversal-order, ownership and coexistence tests; neutral blend-view boundary/refusal suite; complete MFCAD++-2,500 before/after evidence and aggregate transfer comparison. |
...
| `Blend` | One complete same-solid native cylindrical rolling-ball chain. `radius`, canonical full `axis_direction`, dominant `axis`, and the surface leader point `at` describe the occurrence; `side` records the proved `"convex"` external or `"concave"` internal material relation. |
