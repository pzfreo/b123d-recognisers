...
| `recognise_section_passages` | Constant-section line-walled passages on principal or free axes, open at both ends. A complete equal-span wall cycle is preferred; an interrupted cycle may instead be proved by exactly two matching convex polygonal mouths and one same-solid bounded enclosure. Polygon corners follow ordered edge incidence and are independent of individual edge orientation. Mouth-adjacent planar walls are defining and the traversed region is constituent membership. | Capped, tapered, stepped, curved-mouth, one-ended, open/invalid, cross-solid, genuinely shared/multiply opened, or materially obstructed cavities; line/arc schema exists but arc-wall discovery is deferred. Crossed voids already decomposed into historical cycles acquire no additional fallback occurrence. | Independent topology oracle, principal compatibility matrix, interrupted triangular/rectangular/six-sided goldens with mixed edge orientation, compound and ambiguity negatives, arbitrary rigid transforms, STEP, exact Candidate/evidence tests, rejection-gate census, and full MFCAD++ development evidence. |
| `recognise_section_recesses` | Closed, one-ended constant-section pockets with free 3-D frame, line/arc profile, explicit ends and result-local face/body evidence. Current discovery covers proved obround and intact polygonal sections. | Open sections, tapered or stepped cavities, islands, ambiguous ownership, and treatment-interrupted polygonal rings. | Authored reconstruction, covariance, STEP, rejection and compound-ownership tests; complete MFCAD++ development evidence and one-shot aggregate MFInstSeg validation. |
...
| `recognise_prismatic_pockets` | Floored recesses of any planar cross-section, found from a closed equal-span wall ring or from one uniquely bounded inner-mouth region whose partial chamfer/rolling treatment interrupts that ring. The latter reconstructs the section from a direct planar support cycle and proves the complete void, exterior mouth and material-backed floor. Defining evidence remains the planar walls; constituent evidence includes the exact retained floor and mouth-treatment region. | Obround recesses, whose cylindrical ends form no planar support cycle — `recognise_pockets` reaches those; voids open at both ends or capped at both; deeper non-mouth interruptions, branched/broken support cycles, intersecting candidate regions, multiple floors or mouths, breached floors, non-principal openings, and ambiguous/cross-solid ownership. In the aggregate, a four-wall occurrence yields to a paired `Pocket`; a non-rectangular occurrence survives and defeats paired-wall fragments inside it. | Prismatic-pocket functional, partial chamfer/fillet, physical-boundary, STEP, covariance, compound and public-evidence tests; `triangular_and_hex_pockets` golden; paired MFCAD++ development evidence. |
| `recognise_edge_open_prismatic_recesses` | Principal-axis blind polygonal recesses proved by at least three uninterrupted physical planar wall supports forming one non-branching open chain, one exact planar floor, a common axial mouth context, an empty extrusion of that exact floor, material immediately behind it, and one valid solid owner. The section publishes the observed wall chain and the two endpoints of the absent wall without implying geometry across the opening. | Closed pockets; rectangular blind slots; curved, branched, wall-perforated, multiply open, parallel-ended, breached, non-principal, unequal-span, or cross-solid candidates; inferred closing walls or exterior-air footprints. | Authored three/four/five/six-wall positives, schema, physical-boundary, cross-bore, parallel-endpoint, closed-pocket refusal, rigid-axis covariance and compound-locality tests; ADR 0018. |
| `recognise_edge_open_circular_pockets` | Principal-axis blind circular-ended recesses whose two equal-radius cylindrical and two planar supports form one alternating open chain: exactly one arc is a complete semicircle and the interrupted arc is retained only to its physical endpoints. One common mouth, one material-backed floor, equal support spans and one valid solid are required. | Closed obround pockets, through slots, fully erased or doubly interrupted ends, unequal radii, missing floors, non-principal profiles, competing chains and cross-solid joins; no missing arc, closing edge, overall length or closed footprint is inferred. | Authored interruption-position variants, record/golden, closed/through, rigid-axis, reconciliation and compound-ownership tests; complete 2,500-model MFCAD++ development scan; ADR 0018 amendment. |
...
| `EdgeOpenCircularPocket` | One blind circular-ended recess with an interrupted end; it carries only the physical alternating arc/line chain, explicit opening gap and floor-to-mouth interval. |
| `OpenCircularSection` | Four continuous physical arc/line segments in canonical direction plus the explicit gap between their loose endpoints. |
| `OpenCircularSectionSegment` | One original straight or circular section boundary; an arc carries its observed centre, radius and signed sweep. |
| `OpenPolygonalSection` | The canonical open chain of physical wall vertices plus its explicit opening endpoint pair. |
| `OpenSectionOpening` | The two physical endpoints of an absent wall; it implies no joining segment or exterior geometry. |
