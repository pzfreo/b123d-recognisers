...
| `RoundBottomBlindSlot` | One edge-open, one-cap constant U-section blind slot; `axis` and `open_sign` locate its run, `width_axis`/`depth_axis` orient the section, and `flat_width` plus `radius` define its exact profile. |
