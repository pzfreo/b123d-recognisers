...
| --- | --- | --- | --- |
| `recognise_blends` | Complete same-solid chains of native cylindrical rolling-ball patches with one analytic radius, one proved convex/concave material side, two complete support regions and complete original-face evidence. One chain is one occurrence; small and non-principal chains remain visible. Aggregate reconciliation removes a chain only when accepted Fillet occurrences cover all of its curved faces. | Incomplete, branching, cyclic, mixed-radius/side, ambiguous-support or cross-solid components; tori, spheres, surfaces of revolution and recovered/unoriented cylinders. CircularBlindStep and annular Boss/Hole geometry do not satisfy the chain contract. | Authored convex, concave, small-radius, oblique, compound, traversal-order and ownership tests; neutral blend-view boundary/refusal suite; complete MFCAD++-2,500 chain audit and the #414 end-to-end comparison. |
