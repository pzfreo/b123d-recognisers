...
| `recognise_plates` | Body-local thin prismatic slabs supported by opposed planar face clusters and configured area/thickness gates. Equal-valued occurrences on separate valid solids retain multiplicity and independent transverse witnesses. | The single envelope plate, curved/non-prismatic shells, cross-solid face pairing, and slabs below the evidence gates. | Plate/level golden; single/equal/unequal/nested compound, framed rigid-motion, provenance, package and plate boundary tests. |
