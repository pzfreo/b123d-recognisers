...
| `recognise_edge_open_prismatic_recesses` | Principal-axis blind polygonal recesses proved by exactly six physical planar wall supports forming one non-branching open chain, one exact planar floor, a common axial mouth context, an empty extrusion of that exact floor, material immediately behind it, and one valid solid owner. The section publishes the observed wall chain and the two endpoints of the absent wall without implying geometry across the opening. | Closed pockets; rectangular blind slots; curved, branched, multiply open, breached, non-principal, unequal-span, or cross-solid candidates; inferred closing walls or exterior-air footprints. | Authored schema, physical-boundary, closed-pocket refusal, rigid-axis covariance and compound-locality tests; ADR 0018. |
