...
def _validate_record(record: object, family_id: str, index: int) -> str:
    ...
    if record["qualified_name"] != f"b123d_recognisers.{name}":
...
def validate_capability_manifest(manifest: object) -> None:
    ...
    if set(package) != {"name", "version"} or package["name"] != "b123d-recognisers":
        raise CapabilityManifestError("package identity must be b123d-recognisers with a version")
...
def _load_manifest() -> dict[str, Any]:
    resource = files("b123d_recognisers").joinpath("capabilities.json")
