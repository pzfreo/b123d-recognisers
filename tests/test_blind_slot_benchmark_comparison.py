...
@pytest.mark.parametrize("module,fixture,key", [
...
def test_benchmark_ignores_only_projection_changes(module, fixture, key, monkeypatch):
    ...
    assert (measured[False][0].result.section_recesses
            != measured[True][0].result.section_recesses)
    ...
    measured[True] = (replace(product, _legacy_result=replace(product._legacy_result,
                                                             slots=(object(),))), seconds)
