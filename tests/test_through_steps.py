...
def _step(scale: float = 1.0):
    ...
    removal = Pos(15 * scale, 10 * scale, 0) * Box(
        20 * scale, 20 * scale, 30 * scale
    )
...
def _split_upper_terminal(part) -> Solid:
    ...
    solid = Solid(
        Shell([*(face for face in part.faces() if not face.is_same(terminal)), *patches])
    )
    assert solid.is_valid
    return solid
